...
- **Photos** : envoyées vers le bucket privé `clothing-photos` (créé par `supabase/schema.sql`) dès qu'un vêtement est ajouté en étant connectée, sous `{user_id}/{fichier}`. Comme le bucket est privé, l'affichage passe par une URL signée (générée à la demande, valable 1h) plutôt qu'une URL fixe. Si l'envoi échoue (hors ligne), la photo est sauvegardée localement pour ne pas être perdue, et reste visible uniquement sur cet appareil jusqu'à une prochaine synchronisation. **Pas de migration rétroactive** : les vêtements déjà ajoutés avant cette étape (ou ajoutés hors ligne) gardent leur photo en local uniquement.
